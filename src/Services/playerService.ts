...
export async function getPlayers(includes?: P_PlayerIncludes): Promise<({ info: { uuid: string; rttf_score: number | null; playerId: number; } | null; events: { id: number; date_formated: string; title: string | null; isDraft: boolean | null; }[]; profile: { id: number; name: string | null; playerId: number | null; userId: number; } | null; _count: { events: number; }; } & { id: number; name: string; createdAt: Date; updatedAt: Date; profileId: number | null; })[]> {
    let defaultInclude = {
        events: false,
        info: false,
        profile: false
